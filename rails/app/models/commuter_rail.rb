module CommuterRail
  ...
  def self.routes(now = Now.new)
    ...
    results = ActiveRecord::Base.connection.select_all(
      "select routes.gtfs_id, trips.headsign, count(trips.id) as trips_remaining 
       ...
       group by trips.headsign order by gtfs_id asc;").
      group_by {|r| r["gtfs_id"]}.
      map { |route_gtfs_id, values| { :route_short_name  =>  route_gtfs_id.sub(/CR-/, ''),
                                      :headsigns => generate_headsigns(values) }}
  end

  # Only one route_short_names , and the headsigns are the train numbers
  def self.trains(line_name, line_headsign, now = Now.new)
    service_ids = Service.active_on(now.date).map(&:id)
    route = Route.first(:conditions => {:gtfs_id => "CR-#{line_name}"})
    ...
    results = Trip.all(:select => "trips.*, count(stoppings.id) as num_stops",
      ...
      :conditions => ["route_id = ? and headsign like ?
                       and trips.end_time > '#{now.time}' 
                       and trips.service_id in (#{service_ids.join(',')}) ",
                       route.id, "#{line_headsign}%"],
      :group => "stoppings.trip_id",
      :order => "trips.start_time asc"
    ).map {|trip| ["To #{trip.headsign}", 
                   "Starts at #{format_time(trip.start_time)}; 
                   #{trip.num_stops} #{trip.num_stops == 1 ? 'stop' : 'stops'}" ] }
  ...
  def self.trips(options)
    ...
    Trip.all(:joins => :route,
             :conditions => ["routes.gtfs_id = ? and headsign LIKE ? and service_id in (?) and end_time > '#{now.time}'", route_short_name, "#{headsign}%", Service.ids_active_on(now.date)], 
  ...
  def self.arrivals(stopping_id, options)
    now = options[:now] || Now.new
    route_short_name = "CR-#{options[:route_short_name]}"
    route_id = Route.find_by_gtfs_id(route_short_name)
